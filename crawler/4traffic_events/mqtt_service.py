...
def create_configure_client() -> mqtt.Client:
    ...
    user = os.getenv("TTN_EVENTS_APP_ID")
    password = os.getenv("TTN_EVENTS_API_KEY")
...
def connect(client: mqtt.Client) -> None:
    ...
    host = os.getenv("TTN_EVENTS_HOST", None)
    port = int(os.getenv("TTN_EVENTS_PORT", None))
...
def subscribe(client: mqtt.Client) -> None:
    ...
    topic = os.getenv("TTN_EVENTS_TOPIC", None)
